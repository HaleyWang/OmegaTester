...
app.controller('TodoController', function($rootScope, $scope, $http, $timeout) {
    ...
    $scope.clickReqItem = function(node) {
        ...
        var index = _.findIndex($scope.reqTabs , function(o) {return o.id == id});

        var isCurrentTabNotEmpty = $scope.currentReqTabData && !!$scope.currentReqTabData.meta;

        if(isCurrentTabNotEmpty) {
            if(index >= 0) {
                    _.each($scope.reqTabs, function(o) {o.active = null});

                    $scope.reqTabs[index].active = "active";
                    $scope.currentReqTabData = $scope.reqTabs[index];
                    return;
                }

                var reqTab = $scope.reqTabs[$scope.reqTabs.length -1];
                if(reqTab && reqTab.meta) {
                    $scope.reqTabs.push({active: "active", name: "New tab"});

                }
                _.each($scope.reqTabs, function(o) {o.active = null});

                $scope.currentReqTabData = $scope.reqTabs[$scope.reqTabs.length -1];
                $scope.currentReqTabData.active = "active";
        }
    ...
    $scope.treeOptions = {
        accept: function(sourceNodeScope, destNodesScope, destIndex) {
            log("accept-->1 sourceNodeScope", sourceNodeScope.$modelValue, destIndex);
            log("accept-->2 destNodesScope", destNodesScope.$modelValue);
            ...
            var sourceIsGroup = sourceNodeScope.$modelValue.group;
            if(sourceIsGroup && !destNodesScope.$nodeScope) {
                                        return true;
            }
            var destIsGroup = destNodesScope.$nodeScope && destNodesScope.$nodeScope.$modelValue.group;
            if (!sourceIsGroup) {
                log("accept-->2.1 destNodesScope", destNodesScope.$modelValue);
                return destIsGroup;
            }
            if(sourceIsGroup && !destIsGroup) {
                return false;
            }


            var destValue = destNodesScope.$nodesScope.$modelValue ? destNodesScope.$nodesScope.$modelValue.url : null;
            log("accept-->3 destValue", destValue);
